...
static void truncate_caplen(struct filter* filter, int want_sendhead){
	const size_t header_size =
		(sizeof(struct ethhdr) + sizeof(struct sendhead)) * want_sendhead +
...
int mprules_add(const struct filter* filter){
	...
	if ( !discard_filter ){
		...
		truncate_caplen(&rule->filter, con->want_sendhead);
