...
//  struct timeval time;          // arrivaltime
  ...
  while(terminateThreads==0)  {
    ...
    data_len=pcaphead.caplen;
    
    if (PKT_CAPSIZE > 0) {
      //printf ("CAPSIZE %u, data_len=%u, padding=%u\n", PKT_CAPSIZE,data_len,padding);
      if (data_len > PKT_CAPSIZE) {
	data_len = PKT_CAPSIZE;
	padding = 0;
      } else if (data_len < PKT_CAPSIZE) {
	padding = PKT_CAPSIZE - data_len;
      } else
	padding = 0;
      //printf("data_len %u, padding %u\n", data_len,padding);
    }
    memcpy((&datamem[id][writePos][(sizeof(write_head)+sizeof(cap_head))]), payload, data_len);
    
    while (padding --) {
      datamem[id][writePos][(sizeof(write_head)+sizeof(cap_head)) + PKT_CAPSIZE + padding] = 0;
    }
